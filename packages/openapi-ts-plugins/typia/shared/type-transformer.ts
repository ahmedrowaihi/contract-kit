...
const TYPIA_TO_OPENAPI_TARGET: Readonly<Record<string, ReadonlyArray<OpenApiTarget>>> = {
  array: ['array'],
  bigint: ['number'],
  number: ['number'],
  string: ['string'],
...
const TAG_PLAN: Readonly<Record<OpenApiTarget, ReadonlyArray<{ field: string; name: TypiaTagName }>>> =
  (() => {
    const plan: Record<OpenApiTarget, Array<{ field: string; name: TypiaTagName }>> = {
      array: [],
      number: [],
      string: [],
    };
...
function collectTags(schema: IR.SchemaObject, target: OpenApiTarget): Array<ts.TypeNode> {
  ...
  for (const { field, name } of TAG_PLAN[target]) {
    ...
    if (name === 'Type') {
      if (schema.type !== 'integer' || typeof value !== 'string' || !INTEGER_FORMATS.has(value)) {
        continue;
      }
    } else if (name === 'Format') {
...
function buildArrayItemType(ctx: {
  ...
  if (items.length === 1) {
    return typiaTypeTransformer({ ...ctx, schema: items[0]! }) ?? primitiveToTypeNode(items[0]!);
  }
  const mapped = items.map(
    (item) => typiaTypeTransformer({ ...ctx, schema: item }) ?? primitiveToTypeNode(item),
  );
...
function buildTagNode(tagName: TypiaTagName, value: number | string | null): ts.TypeNode {
  const typeArgs =
    value === null
      ? undefined
      : [
