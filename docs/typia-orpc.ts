...
export interface TypiaSchema<Input = unknown, Output = Input> {
  readonly '~standard': {
    ...
    readonly validate: (value: unknown) =>
      | { value: Output }
      | { issues: ReadonlyArray<{ message: string; path?: ReadonlyArray<{ key: string | number }> }> }
 ...
 * )
 */
export function createTypiaSchema<S extends { '~standard': { version: 1; vendor: string; validate: (value: unknown) => any } }>(
  standardSchema: S,
  collection: TypiaJsonSchemaCollection,
